SELECT  msc_telp_type
        ...
        ,msc_log_rsn
        ,msc_sms_type
FROM u_msc inner JOIN SQL000: ORA-Migration Range t1 
	on substr( u_msc.msc_called_number, 2 , 20) >= t1.range_from 
	and substr( u_msc.msc_called_number, 2 , 20) <= t1.range_to
WHERE (
		date (partition_col) >=  date( '2021-12-05' )
		AND date (partition_col) <= date( '2021-12-05' ) 
-- WHERE (date_format(partition_col, 'yyyy-MM-dd') >=  '2021-12-05' 
		-- AND date_format(partition_col, 'yyyy-MM-dd') <=   '2021-12-05' 
		AND msc_part_key IN (00, 01)) --//! msc_part_key no values
